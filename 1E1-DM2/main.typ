#import "../templates/dm.typ": dm, defunc
...
Posons $m = 0$, on a $g(n) = g(0) times g(n) = 0$. Donc $forall n in NN, g(n) = 0$.

Donc $g$ est bien la fonction nulle.
...
Pour $n=0$, $g(n) = g(0) = 1 = a^0$.

- #underline[*Hérédité*] :

Soit $n in NN$.
Supposons que $g(n) = a^n$

Montrons que $g(n+1) = a^(n+1)$ :
...
- #underline[*Conclusion*] : Donc $forall n in NN$, $g(n) = a^n$ avec $a = g(1)$, d'après le principe de récurrence.
...
Supposons que $m=n=0$, on a donc $f(0)=f(0)^2+2f(0)$

Donc $-f(0)=f(0)^2$. Donc $f(0)=0$ car $f(0) in NN$
...
====
Soient $n, m in NN$. On pose $g(a)=f(a)+1$ pour tout $a in NN$. On a alors :
...
Donc, la fonction #defunc($f$, $NN$, $NN$, $n$, $2^n - 1$) est la seule fonction solution de l'équation fonctionnelle.
